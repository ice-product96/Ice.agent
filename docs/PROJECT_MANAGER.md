...
- `ProjectState` хранит конфигурацию и уровень автономности отдельного проекта. Уровни варьируются от `LEVEL_0` до `LEVEL_3`; новый проект по умолчанию получает `LEVEL_1`.
  В `config` также настраиваются рабочие часы (`timezone`, `workday_start`/`workday_end`, `workdays`), ставка (`hourly_rate`, `currency`), флаг `cost_requires_customer_approval` и `min_execution_ratio` (доля оценки, которую Cursor обязан «отработать» до QA accept). По умолчанию часовой пояс проекта — `Asia/Yekaterinburg` (UTC+5).
...
### Рабочие часы, оценка и стоимость

- Вне рабочих часов проекта агент может принимать сообщения, обсуждать и согласовывать; `submit_development_task` откладывает Cursor до ближайшего открытия смены (`wait_until` + one-shot cron).
- `pm_estimate_task` / `estimated_duration_minutes` в `pm_structure_task` пишут оценку длительности, `min_execution_minutes` и `estimated_cost = rate × hours`.
- Если `cost_requires_customer_approval=true`, до Cursor нужно `pm_record_decision` по теме стоимости.
- `pm_accept_task` отклоняет закрытие, пока не истекло минимальное время по оценке (по сумме `CursorRun.started_at`…`completed_at`).
- В карточке кейса UI показывает расписание, следующее событие, оценку/стоимость и настройки проекта.
...
- `pm_poll_tracker` — незакрытые карточки ice_tracker для проектов с `tracker_project_id` против уже существующих кейсов.

В карточке заказчика задайте UUID `tracker_project_id` и включите опрос. На heartbeat при `pm_mode` платформа сама смотрит доску; если есть карточки без `WorkItem` (ключ `context_json.tracker_task_id`), тик не пропускается и агент берёт **одну** через `pm_structure_task`. Дальше — обычные гейты автономности/оценки/часов.
...
Работы с высоким риском и работы за пределами согласованной области по-прежнему требуют разрешения. Коммерческие условия, серьёзные обязательства по срокам, конфликты области работ, разрушительные действия в production, инциденты безопасности, удаление production-данных и изменения биллинга необходимо эскалировать руководителю. Обычную разработку согласует менеджер проектов с заказчиком: `pm_record_decision` с подтверждением заказчика переводит задачу в `CLIENT_CONFIRMED` и позволяет `submit_development_task` даже на последующем тике. Состояние высокого риска `owner_approved` принимается только из сохранённой консультации со статусом `approved`; его не используют для обычных задач заказчика.
