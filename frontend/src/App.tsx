...
function RuntimeScreen() {
  ...
  return <form className="settings-layout runtime-layout" onSubmit={save}>
    ...
    <section className="panel"><SectionHead title="Выполнение задач" text="Параллелизм и лимиты цикла инструментов"/><div className="form-grid"><Field label="Воркеры задач"><input required min="1" type="number" value={form.task_workers} onChange={e => number('task_workers', e.target.value)}/></Field><Field label="Максимум раундов инструментов" hint="1–200. Сложные PM-задачи часто нуждаются в 30–80."><input required min="1" max="200" type="number" value={form.max_tool_rounds} onChange={e => number('max_tool_rounds', e.target.value)}/></Field></div></section>
