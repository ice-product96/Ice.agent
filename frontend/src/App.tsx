  ...
  return <Modal title={form.id ? 'Настройка агента' : 'Создание агента'} subtitle="Задайте личность, runtime и взаимодействие." onClose={onClose}>
    <form onSubmit={submit}>
      ...
      <div className="form-grid">
        ...
        <Field label="Realtime model" hint="Модель Realtime-сессии (по умолчанию gpt-realtime-2). HTTP-прокси — из профиля LLM агента."><input value={form.realtime_model || 'gpt-realtime-2'} onChange={e => patch({ realtime_model: e.target.value })} placeholder="gpt-realtime-2"/></Field>
...
function LlmProfileForm({ value, onClose, onSave }: { value: Partial<LlmProfile> | LlmProfileWrite; onClose: () => void; onSave: (v: Partial<LlmProfileWrite>, id?: string) => Promise<void> }) {
  ...
  return <Modal title={id ? 'Изменение профиля LLM' : 'Новый профиль LLM'} subtitle="Ключи только для записи и не загружаются обратно в форму." onClose={onClose}><form onSubmit={async e => {
    ...
    {error && <Alert message={error}/>}<div className="form-grid">
      ...
      <Field label="HTTP-прокси OpenAI" hint="Обязателен для SIP Realtime из Docker/RU: WSS идёт через HTTP CONNECT (как в Mtz). Пример: http://user:pass@host:8080" wide><input value={form.http_proxy || ''} onChange={e => patch({ http_proxy: e.target.value })} placeholder="http://127.0.0.1:8080" autoComplete="off"/></Field>
